  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 text-center">
        ...
        <p className="text-neutral-900 font-semibold text-lg mb-2 italic">Start a conversation to generate React components</p>
        <p className="text-neutral-500 text-sm max-w-sm italic">I can help you create buttons, forms, cards, and more</p>
      </div>
    );
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const filePath = args?.path ? args.path.split("/").pop() : null;
                            const toolLabel = (() => {
                              if (tool.toolName === "str_replace_editor") {
                                if (args?.command === "create") return `Creating ${filePath ?? "file"}`;
                                if (args?.command === "str_replace") return `Editing ${filePath ?? "file"}`;
                                if (args?.command === "view") return `Reading ${filePath ?? "file"}`;
                              }
                              if (tool.toolName === "file_manager") {
                                if (args?.command === "delete_file") return `Deleting ${filePath ?? "file"}`;
                                if (args?.command === "rename_file") return `Renaming ${filePath ?? "file"}`;
                                if (args?.command === "create_file") return `Creating ${filePath ?? "file"}`;
                              }
